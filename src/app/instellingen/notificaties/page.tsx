  ...
  return (
    <div style={{ minHeight: '100vh', background: 'var(--bg-app)' }}>
      ...
      <main style={{ padding: '36px 40px 72px', maxWidth: 600, margin: '0 auto' }}>
        ...
        <p style={{ fontSize: 13, color: '#9CA3AF', marginBottom: 28 }}>Beheer jouw herinneringen en nudges</p>
        ...
        {laden ? (
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
            ...
            {OPTIES.map(opt => (
              <div key={opt.key} style={{ background: 'white', borderRadius: 16, border: '1px solid #E5E7EB', padding: '18px 20px' }}>
                <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: 12 }}>
                  <div>
                    <p style={{ fontSize: 14, fontWeight: 600, color: '#111827', marginBottom: 3 }}>{opt.label}</p>
                    <p style={{ fontSize: 12, color: '#9CA3AF', lineHeight: 1.5 }}>{opt.beschrijving}</p>
                  </div>
                  <button
                    onClick={() => setVoorkeuren(prev => ({ ...prev, [opt.key]: !prev[opt.key] }))}
                    style={{
                      width: 44, height: 24, borderRadius: 100, border: 'none', cursor: 'pointer',
                      background: voorkeuren[opt.key] ? opt.kleur : '#E5E7EB',
            ...
            <div style={{ background: 'white', borderRadius: 16, border: '1px solid #E5E7EB', padding: '18px 20px' }}>
              <p style={{ fontSize: 14, fontWeight: 600, color: '#111827', marginBottom: 6 }}>Voorkeurstijdstip</p>
              <p style={{ fontSize: 12, color: '#9CA3AF', marginBottom: 12 }}>Op welk moment wil je herinneringen ontvangen?</p>
              <input
                type="time"
                value={voorkeuren.reminder_tijd}
                onChange={e => setVoorkeuren(prev => ({ ...prev, reminder_tijd: e.target.value }))}
                style={{ border: '1px solid #E5E7EB', borderRadius: 10, padding: '9px 14px', fontSize: 14, outline: 'none', color: '#374151' }}
              />
            </div>

    ...
    <div style={{ minHeight: '100vh', background: 'var(--bg-app)' }}>
      ...
      <main style={{ padding: '24px 20px 88px', maxWidth: 600, margin: '0 auto' }}>
        ...
        <header style={{ marginBottom: 24 }}>
          ...
          <p style={{ fontSize: 13, color: '#9CA3AF' }}>Log je slaap en ontdek patronen</p>
        </header>
        ...
        {(gemiddeldUren !== null || gemiddeldKwaliteit !== null) && (
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10, marginBottom: 20 }}>
            {gemiddeldUren !== null && (
              <div style={{ background: 'white', borderRadius: 16, padding: '16px', border: '1px solid #E5E7EB' }}>
                ...
                <p style={{ fontSize: 10, color: '#9CA3AF', marginTop: 2 }}>afgelopen 2 weken</p>
              </div>
            )}
        ...
        <div style={{ background: 'white', borderRadius: 20, padding: '20px', border: '1px solid #E5E7EB', marginBottom: 20 }}>
          ...
          <div style={{ marginBottom: 14 }}>
            <label style={{ fontSize: 11, fontWeight: 600, color: '#6B7280', display: 'block', marginBottom: 6 }}>Datum</label>
          ...
          <div style={{ marginBottom: 14 }}>
            <label style={{ fontSize: 11, fontWeight: 600, color: '#6B7280', display: 'block', marginBottom: 8 }}>Kwaliteit</label>
            <div style={{ display: 'flex', gap: 8 }}>
              {[1, 2, 3, 4, 5].map(k => (
                <button key={k} onClick={() => setKwaliteit(k)} style={{
                  flex: 1, height: 40, borderRadius: 10, border: 'none', cursor: 'pointer',
                  background: kwaliteit === k ? '#111827' : '#F3F4F6',
            ...
            <p style={{ fontSize: 10, color: '#9CA3AF', marginTop: 4, textAlign: 'center' }}>{KWALITEIT_LABELS[kwaliteit]}</p>
          </div>

          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10, marginBottom: 14 }}>
            <div>
              <label style={{ fontSize: 11, fontWeight: 600, color: '#6B7280', display: 'block', marginBottom: 6 }}>Bedtijd</label>
              <input type="time" value={bedtijd} onChange={e => setBedtijd(e.target.value)}
                style={{ width: '100%', padding: '10px 12px', borderRadius: 10, border: '1.5px solid #E5E7EB', fontSize: 13, boxSizing: 'border-box' }} />
            </div>
            <div>
              <label style={{ fontSize: 11, fontWeight: 600, color: '#6B7280', display: 'block', marginBottom: 6 }}>Wektijd</label>
          ...
          <>
            ...
            <div style={{ background: 'white', borderRadius: 16, padding: '16px', border: '1px solid #E5E7EB', marginBottom: 16 }}>
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: 4 }}>
                <span style={{ fontSize: 9, color: '#9CA3AF' }}>14 dagen geleden</span>
                <span style={{ fontSize: 9, color: '#9CA3AF' }}>Vandaag</span>
              </div>
            </div>
              ...
              {logs.slice(0, 7).map(log => (
                <div key={log.id} style={{
                  ...
                  <div style={{ flex: 1 }}>
                    ...
                    {log.kwaliteit && <p style={{ fontSize: 10, color: '#9CA3AF' }}>

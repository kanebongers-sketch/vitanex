...
export default function VoortgangPage() {
  ...
  const METRICS = [
    ...
    { label: 'Dankbaar', waarde: totalen.dankbaarheid, eenheid: 'Ã—', kleur: '#EC4899', bg: '#FCE7F3', href: '/dankbaarheid' },
    ...
    <div style={{ minHeight: '100vh', background: 'var(--bg-app)' }}>
      ...
      <main style={{ padding: '36px 40px 72px', maxWidth: 900, margin: '0 auto' }}>
        ...
        <div style={{ marginBottom: 28 }}>
          <h1 style={{ fontSize: 22, fontWeight: 800, color: '#111827', letterSpacing: '-0.03em', marginBottom: 4 }}>Mijn voortgang</h1>
          <p style={{ fontSize: 13, color: '#9CA3AF' }}>Jouw welzijns-statistieken van de afgelopen 30 dagen</p>
        </div>
          ...
          <>
            ...
            {streak && (
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 12, marginBottom: 20 }}>
                {[
                  { label: 'Huidige streak', waarde: streak.huidige_streak, suffix: 'dagen', kleur: '#1D9E75', icon: 'ðŸ”¥' },
                  { label: 'Langste streak', waarde: streak.langste_streak, suffix: 'dagen', kleur: '#F59E0B', icon: 'ðŸ†' },
                  { label: 'Totaal actief', waarde: streak.totaal_dagen, suffix: 'dagen', kleur: '#6366f1', icon: 'ðŸ“…' },
                ].map(item => (
                  <div key={item.label} style={{ background: 'white', borderRadius: 16, border: '1px solid #E5E7EB', padding: '20px 22px' }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 8 }}>
                      <span style={{ fontSize: 18 }}>{item.icon}</span>
                      <p style={{ fontSize: 11, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.06em', color: '#9CA3AF' }}>{item.label}</p>
                    </div>
                    <p style={{ fontSize: 28, fontWeight: 800, color: item.kleur }}>{item.waarde}</p>
                    <p style={{ fontSize: 12, color: '#9CA3AF' }}>{item.suffix}</p>
                  </div>
                ))}
              </div>
            )}
                  ...
                  <thead>
                    <tr>
                      {['Week', 'Stemming', 'Slaap', 'Stress', 'Focus', 'Check-ins'].map(h => (
                        <th key={h} style={{ textAlign: 'left', fontSize: 11, fontWeight: 700, color: '#9CA3AF', paddingBottom: 10, whiteSpace: 'nowrap', paddingRight: 20 }}>{h}</th>
                      ))}
                    </tr>
                  </thead>
                  <tbody>
                    {weekStats.map((w, i) => (
                      <tr key={i} style={{ borderTop: '1px solid #F3F4F6' }}>
                        <td style={{ padding: '10px 20px 10px 0', fontSize: 12, color: '#6B7280', fontWeight: 600, whiteSpace: 'nowrap' }}>{w.week}</td>
                        <td style={{ padding: '10px 20px 10px 0' }}>
                          {w.stemming !== null
                            ? <span style={{ fontSize: 13, fontWeight: 700, color: w.stemming >= 4 ? '#1D9E75' : w.stemming >= 3 ? '#F59E0B' : '#E24B4A' }}>{w.stemming}/5</span>
                            : <span style={{ fontSize: 12, color: '#D1D5DB' }}>â€”</span>}
                        </td>
                        <td style={{ padding: '10px 20px 10px 0' }}>
                          {w.slaap !== null
                            ? <span style={{ fontSize: 13, fontWeight: 700, color: w.slaap >= 7 ? '#1D9E75' : w.slaap >= 5 ? '#F59E0B' : '#E24B4A' }}>{w.slaap}u</span>
                            : <span style={{ fontSize: 12, color: '#D1D5DB' }}>â€”</span>}
                        </td>
                        <td style={{ padding: '10px 20px 10px 0' }}>
                          {w.stress !== null
                            ? <span style={{ fontSize: 13, fontWeight: 700, color: w.stress <= 4 ? '#1D9E75' : w.stress <= 6 ? '#F59E0B' : '#E24B4A' }}>{w.stress}/10</span>
                            : <span style={{ fontSize: 12, color: '#D1D5DB' }}>â€”</span>}
                        </td>
                        <td style={{ padding: '10px 20px 10px 0' }}>
                          <span style={{ fontSize: 13, fontWeight: 700, color: w.focus >= 60 ? '#6366f1' : '#9CA3AF' }}>{w.focus}m</span>
                        </td>
                        <td style={{ padding: '10px 20px 10px 0' }}>
                          <div style={{ display: 'flex', gap: 3 }}>
                            {Array.from({ length: 5 }, (_, j) => (
                              <div key={j} style={{ width: 8, height: 8, borderRadius: 2, background: j < w.checkins ? '#1D9E75' : '#E5E7EB' }} />
                            ))}
                          </div>
                        </td>
                      </tr>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: 12 }}>
              {[
                { href: '/inzichten', label: 'AI Inzichten', tekst: 'Persoonlijke analyse van jouw week', kleur: '#6366f1', icon: 'âœ¨' },
                { href: '/groeiplan', label: 'Groeiplan', tekst: 'AI-gegenereerd persoonlijk groeiplan', kleur: '#1D9E75', icon: 'ðŸŒ±' },
                { href: '/mentale-sterkte', label: 'Mentale sterkte', tekst: 'Quiz: hoe sterk ben jij mentaal?', kleur: '#8B5CF6', icon: 'ðŸ§ ' },
                { href: '/achievements', label: 'Achievements', tekst: 'Bekijk jouw behaalde badges', kleur: '#F59E0B', icon: 'ðŸ…' },
              ].map(item => (
                <Link key={item.href} href={item.href} style={{ textDecoration: 'none', background: 'white', borderRadius: 16, border: '1px solid #E5E7EB', padding: '18px 20px', display: 'flex', alignItems: 'center', gap: 14 }}>
                  ...
                  <div>
                    <p style={{ fontSize: 14, fontWeight: 700, color: '#111827', marginBottom: 2 }}>{item.label}</p>
                    <p style={{ fontSize: 12, color: '#9CA3AF' }}>{item.tekst}</p>
                  </div>
                  <svg style={{ marginLeft: 'auto', flexShrink: 0 }} width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="#9CA3AF" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><polyline points="9 18 15 12 9 6"/></svg>
                </Link>
              ))}
            </div>
          </>

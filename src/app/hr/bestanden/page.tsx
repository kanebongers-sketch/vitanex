      ...
      <div style={{ maxWidth: 900, margin: '0 auto', padding: '32px 20px' }}>
        ...
        <div style={{ display: 'flex', gap: 4, marginBottom: 28, background: '#0a1628', borderRadius: 10, padding: 4, width: 'fit-content' }}>
          {(['bestanden', 'rapporten'] as const).map(tab => (
            <button key={tab} onClick={() => setActieveTab(tab)} style={{ background: actieveTab === tab ? '#1e40af' : 'transparent', color: actieveTab === tab ? '#fff' : 'var(--text-2)', border: 'none', borderRadius: 8, padding: '8px 20px', fontSize: 14, fontWeight: 600, cursor: 'pointer' }}>

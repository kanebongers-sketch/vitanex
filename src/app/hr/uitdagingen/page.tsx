      ...
      <main style={{ padding: '24px 20px 88px', maxWidth: 600, margin: '0 auto' }}>
        ...
        <header style={{ marginBottom: 24, display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
          <div>
            ...
            <p style={{ fontSize: 13, color: '#9CA3AF' }}>Maak en beheer team wellbeing challenges</p>
          </div>
        ...
        {showForm && (
          <div style={{ background: 'white', borderRadius: 20, padding: '20px', border: '1px solid #E5E7EB', marginBottom: 20 }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
              <div>
                <label style={{ fontSize: 11, fontWeight: 600, color: '#6B7280', display: 'block', marginBottom: 4 }}>Titel *</label>
              ...
              <div>
                <label style={{ fontSize: 11, fontWeight: 600, color: '#6B7280', display: 'block', marginBottom: 4 }}>Beschrijving</label>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
                <div>
                  <label style={{ fontSize: 11, fontWeight: 600, color: '#6B7280', display: 'block', marginBottom: 4 }}>Doelwaarde</label>
                  <input type="number" value={form.doel_waarde} onChange={e => setForm(p => ({ ...p, doel_waarde: e.target.value }))}
                    placeholder="10000"
                    style={{ width: '100%', padding: '10px 12px', borderRadius: 10, border: '1.5px solid #E5E7EB', fontSize: 13, boxSizing: 'border-box' }} />
                </div>
                <div>
                  <label style={{ fontSize: 11, fontWeight: 600, color: '#6B7280', display: 'block', marginBottom: 4 }}>Eenheid</label>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
                <div>
                  <label style={{ fontSize: 11, fontWeight: 600, color: '#6B7280', display: 'block', marginBottom: 4 }}>Startdatum</label>
                  <input type="date" value={form.start_datum} onChange={e => setForm(p => ({ ...p, start_datum: e.target.value }))}
                    style={{ width: '100%', padding: '10px 12px', borderRadius: 10, border: '1.5px solid #E5E7EB', fontSize: 13, boxSizing: 'border-box' }} />
                </div>
                <div>
                  <label style={{ fontSize: 11, fontWeight: 600, color: '#6B7280', display: 'block', marginBottom: 4 }}>Einddatum *</label>
              ...
              <button onClick={aanmaken} disabled={opslaan || !form.titel.trim() || !form.eind_datum} style={{
                ...
                background: opslaan ? '#9CA3AF' : '#1D9E75',
              ...
              return (
                <div key={u.id} style={{
                  ...
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 8 }}>
                    <div style={{ flex: 1 }}>
                      <p style={{ fontSize: 14, fontWeight: 700, color: '#111827', marginBottom: 2 }}>{u.titel}</p>
                      {u.beschrijving && <p style={{ fontSize: 12, color: '#9CA3AF' }}>{u.beschrijving}</p>}
                    </div>
                    <div style={{ flexShrink: 0, textAlign: 'right', marginLeft: 12 }}>
                      {resterende > 0 ? (
                        <span style={{ fontSize: 10, fontWeight: 700, color: '#1D9E75', background: '#F0FDF4', padding: '2px 8px', borderRadius: 99 }}>
                          {resterende}d resterend
                        </span>
                      ) : (
                        <span style={{ fontSize: 10, fontWeight: 700, color: '#9CA3AF', background: '#F3F4F6', padding: '2px 8px', borderRadius: 99 }}>
                          Afgelopen
                        </span>
                      )}
                    </div>
